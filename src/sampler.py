...
class LossSampler(data.Sampler[int]):
    ...
    def update(self, state: TrainState) -> None:
        ...
        # torch.save(self.loss_weights, "../results/distr/loss_inv_distr.pth")
...
class GradnormSampler(data.Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        perm = torch.multinomial(
            self.gradnorm_weights, self.data_len, self.replacement, generator=self.rng
        )
        yield from perm.tolist()
