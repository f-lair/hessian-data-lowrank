...
def main() -> None:
    parser = ArgumentParser("GGN Sample-Approximation Training Script")
    ...
    # Setup checkpointing
    max_steps = args.epochs * (len(train_dataset) // args.train_batch_size) - 1  # type: ignore
    ...
    checkpoint_options = CheckpointManagerOptions(
        save_interval_steps=args.checkpoint_interval, save_on_steps=[max_steps], create=True
    )
